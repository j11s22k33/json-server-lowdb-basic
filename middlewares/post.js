...
handlers.getAllPost = async (req, res) => {
  const rows = db.helper.get(DBNAME_POSTS).sortBy("_id").reverse().value();

  res.status(200).json(rows);
};
handlers.getPostByID = async (req, res) => {
  const id = req.params.id;
  const row = db.helper.get(DBNAME_POSTS).find({ _id: id }).value();
...
handlers.deletePost = async (req, res) => {
  ...
  const rows = db.helper.get(DBNAME_POSTS).remove({ _id: id }).write();
